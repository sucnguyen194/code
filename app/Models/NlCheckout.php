...
class NlCheckout extends Model
{
    use HasFactory;
    ...
    public function checkout($order){
        $receiver= setting('checkout.nl_receiver');
        //Mã đơn hàng
        $order_code='NL_'.time();
        //Khai báo url trả về
        $return_url= route('order.nl.success');
        // Link nut hủy đơn hàng
        $cancel_url= $_SERVER['HTTP_REFERER'];
        $notify_url = route('order.nl.success');
        //Giá của cả giỏ hàng
...
//        $nl->nganluong_url = NGANLUONG_URL;
        $this->merchant_site_code = setting('checkout.nl_merchant_id');
        $this->secure_pass = setting('checkout.nl_merchant_pass');
        //Tạo link thanh toán đến nganluong.vn
        $url= $this->buildCheckoutUrlExpand($return_url, $receiver, $transaction_info, $order_code, $order->vnd, $currency, $quantity, $tax, $discount , $fee_cal,    $fee_shipping, $order_description, $buyer_info , $affiliate_code);
        //$url= $nl->buildCheckoutUrl($return_url, $receiver, $transaction_info, $order_code, $price);

        if(!$order_code)
            return flash('Error: Url not found', 0);

        $order->payment_type = PaymentType::NganLuong;
        $order->payment_code = $order_code;
        $order->save();
